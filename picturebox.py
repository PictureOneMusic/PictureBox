...
class Sound:
    ...
    def frames2array(self, data, sampwidth, numchan):
        if sampwidth == 2:
            ...
            npdata = picturebox_audio.binary24_to_int16(data, len(data)/3)
        if numchan == 1:
...
def AudioCallback(outdata, frame_count, time_info, status):
    ...
    b = picturebox_audio.mixaudiobuffers(playingsounds, rmlist, frame_count, FADEOUT, FADEOUTLENGTH, SPEED)
    for e in rmlist:
        try:
